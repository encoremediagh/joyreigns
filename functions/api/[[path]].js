...
export async function onRequest({request,env,params}){
  ...
  if(method==='GET'&&resource==='hero-slides'&&!idPart)return json((await env.DB.prepare("SELECT * FROM hero_slides WHERE status='published' ORDER BY position ASC, created_at DESC").all()).results);
  ...
  if(method==='GET'&&resource==='hero'&&!idPart)return json((await env.DB.prepare('SELECT * FROM hero_slides ORDER BY position ASC, created_at DESC').all()).results);
  ...
  if(method==='POST'&&resource==='hero'){
    const v=await body(request);if(!v.headline||!v.image_url)return json({error:'Headline and background image are required.'},400);
    await env.DB.prepare('INSERT INTO hero_slides(eyebrow,headline,subtitle,image_url,primary_label,primary_url,secondary_label,secondary_url,position,status) VALUES(?,?,?,?,?,?,?,?,?,?)').bind(v.eyebrow||'Trusted real estate guidance in Ghana',v.headline,v.subtitle||'',v.image_url,v.primary_label||'Explore properties',v.primary_url||'#properties',v.secondary_label||'',v.secondary_url||'',Number(v.position)||0,v.status||'draft').run();
    return json({ok:true},201);
  }
  ...
  if(method==='PATCH'&&resource==='hero'&&idPart){
    const v=await body(request),id=Number(idPart);if(!id)return json({error:'Invalid id.'},400);
    await env.DB.prepare('UPDATE hero_slides SET eyebrow=?,headline=?,subtitle=?,image_url=?,primary_label=?,primary_url=?,secondary_label=?,secondary_url=?,position=?,status=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(v.eyebrow||'',v.headline,v.subtitle||'',v.image_url||'',v.primary_label||'',v.primary_url||'',v.secondary_label||'',v.secondary_url||'',Number(v.position)||0,v.status||'draft',id).run();return json({ok:true});
  }
  ...
  if(method==='DELETE'&&resource==='hero'&&idPart){await env.DB.prepare('DELETE FROM hero_slides WHERE id=?').bind(Number(idPart)).run();return json({ok:true});}
